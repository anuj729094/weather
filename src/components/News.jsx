...
const News = ({ obj }) => {
    ...
    useEffect(() => {
        ...
        }).catch((error) => setNewsError("Something went wrong"))
    }, [obj])
    return (
        <div className='flex flex-col items-center px-3 lg:px-12 xl:px-0'>
            <div className=' xl:w-[75rem]'>
                {loading ?
                    <Newsskeleton /> :
                    newsError ? <h1 className=' text-sm sm:text-lg '>{newsError}</h1> :
                        <div>
                            <h3 className=' text-2xl sm:text-4xl mb-5 font-medium '>{obj.type}</h3>
