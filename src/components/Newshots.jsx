    ...
    <div id="hots" className=' scroll-mt-24 xl:flex xl:justify-center px-3 lg:px-12 xl:px-0'>
      <div className=' xl:w-[75rem]'>
        ...
        <ul className=' hots inline-flex whitespace-nowrap gap-3 overflow-auto w-full my-5'>
          {
            arr.map((list, index) => <li style={list.split(" ")[0].toLowerCase() === type ? { backgroundColor: "#E10C0D", color: 'white' } : {}} key={index}><button onClick={() => setobj({type: list.split(" ")[0].toLowerCase(), location: country || "India"
            })}>{list}</button></li>)
          }
        </ul>
      </div>
